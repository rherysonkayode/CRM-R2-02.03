...
import { Users, Building2, Handshake, DollarSign, TrendingUp, Trophy, Calendar, Plus, ArrowRight } from "lucide-react";
...
// Interface temporária para profile (remova quando os tipos estiverem gerados)
...
const Dashboard = () => {
  ...
  // Ranking de corretores (só para imobiliária)
  ...
  // Contagem de corretores ativos (para imobiliária)
  ...
  return (
    <AppLayout>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        {/* Cabeçalho com saudação e ações rápidas */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            <Button asChild size="sm" className="bg-[#7E22CE] hover:bg-[#6b21a8]">
              <Link to="/funnel">
                <TrendingUp className="w-4 h-4 mr-2" />Ver Funil
              </Link>
            </Button>
          </div>
        </div>
        ...
        {/* Grid de duas colunas: Funil e Ranking/Eventos */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Gráfico do Funil (ocupa 2 colunas em telas grandes) */}
          ...
          {/* Coluna da direita: Ranking ou Calendário */}
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-lg font-semibold flex items-center gap-2">
                {isImobiliaria ? <Trophy className="w-5 h-5 text-amber-500" /> : <Calendar className="w-5 h-5 text-primary" />}
                {isImobiliaria ? "Ranking de Corretores" : "Compromissos Hoje"}
              </CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
        ...
        {/* Grid de três colunas: Leads Recentes + Card de Equipe */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Leads Recentes (ocupa 2 colunas em telas grandes) */}
          <Card className="lg:col-span-2">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          {/* Card de corretores ativos (apenas para imobiliária) */}
          {isImobiliaria && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-lg font-semibold flex items-center gap-2">
